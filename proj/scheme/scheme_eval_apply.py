...
def scheme_eval(expr, env, tail=False): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        args_evaluated = rest.map(lambda exp : scheme_eval(exp, env))
        return scheme_apply(operator, args_evaluated, env)
        # END PROBLEM 3
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        try:
            # BEGIN PROBLEM 2
            "*** YOUR CODE HERE ***"
            return procedure.py_func(*args_py) # here
            # END PROBLEM 2
...
def eval_all(expressions, env):
    ...
    while expr.rest is not nil:
        scheme_eval(expr.first, env, tail=False)
        expr = expr.rest
    return scheme_eval(expr.first, env, tail=True)
    # END PROBLEM 6
