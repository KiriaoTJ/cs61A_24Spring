...
class VendingMachine:
    ...
    def restock(self, amount):
        self.storage += amount
        return f'Current {self.good} stock: {self.storage}'
    def vend(self):
        if self.storage == 0:
...
def two_list(vals, counts):
    ...
    def repeat(num, cnt):
        if cnt == 1:
            ...
            return Link(num, repeat(num, cnt - 1))
    if len(vals) == len(counts) == 1:
        return repeat(vals[0], counts[0])
    else:
        head = repeat(vals[0], counts[0])
        tail = head
        while tail.rest is not tail.empty:
            tail = tail.rest
