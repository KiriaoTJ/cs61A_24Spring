...
class Account:
    ...
    def deposit(self, amount):
        ...
        if len(self.transactions) == 0:
            self.transactions.append(Transaction(0, self.balance, self.balance + amount))
        else:
            self.transactions.append(Transaction(self.transactions[-1].id + 1, self.balance, self.balance + amount))
    ...
    def withdraw(self, amount):
        ...
        if amount > self.balance:
            self.transactions.append(Transaction(self.transactions[-1].id + 1, self.balance, self.balance))
...
def make_change(amount, coins):
    ...
    if amount == smallest:
        return [smallest]
    after = make_change(amount - smallest, rest)
    if type(after) == list:
